...
6. Update task and plan files immediately; do not defer state sync. Name new detail files `<timestamp>-<feature-slug>.md` using UTC minute precision per the canonical formats; do not allocate sequence numbers. Task and plan records may be revised or deleted; preserve change history in `docs/changelog.md`.
