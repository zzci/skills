...
4. **Deliver the developer-handoff folder by path.** There is no download tool in this environment — the folder already lives in the user's filesystem. Give the user its absolute path and summarize its contents; if they want a portable archive (to attach or send elsewhere), zip it with `cd <project-folder> && zip -r design_handoff_<feature-name>.zip design_handoff_<feature-name>/` and give the zip's path.
