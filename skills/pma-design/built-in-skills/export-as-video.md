...
Render a finished timeline animation to a real video file (`.mp4`, `.webm`, or `.gif`). One run of the bundled **gen-video CLI** drives a headless Chromium, seeks the animation frame-by-frame through its timeline bridge, and pipes the frames to **ffmpeg**.
...
3. **Write the config JSON** (below) and **run the gen-video CLI**: `node <skill>/agents/gen-video/dist/cli.mjs --url <servedUrl> --config <jsonPath> --out designs/<project>`. Full invocation + one-time setup (npm install, Playwright Chromium, ffmpeg): your harness reference → "Exporting to video".
...
Frame count is `fps × (endMs − startMs)`, and every frame is a real headless screenshot, so long high-fps exports take minutes — pick `startMs`/`endMs` to iterate on a section, then run the full range. Then surface the file per your harness reference (give the absolute path; use a show-file capability if one is available).
