...
Export an HTML slide deck to a `.pptx` with native PowerPoint objects (editable text, shapes, images). One run of the **gen-pptx CLI** (bundled with this skill) does everything: capture, font handling, generation, output file.
...
3. **Run the gen-pptx CLI** with the inputs below: serve the deck over HTTP (the shared nsl `designs` server), write the inputs to a JSON file, then `node <skill>/agents/gen-pptx/dist/cli.mjs --url <servedDeckUrl> --config <jsonPath> --out designs/<project>`. Full invocation + one-time setup (npm install, Playwright Chromium, build): your harness reference → "Exporting to PPTX".
4. **Read the validation flags** in the printed result JSON and decide if you need to retry.

## Exporter inputs (the config JSON)
...
```jsonc
{
  ...
  // The exporter clears the transform AND forces width/height onto this element.
...
If the flags look like real problems, fix the inputs and retry. If they're expected (deck genuinely has no notes, two slides really are identical), surface the `.pptx` file to the user (path + your harness's show-file capability) and move on.

**Talking to the user about flags:** these names and messages are internal diagnostics — do NOT relay them verbatim. If everything is expected, don't mention validation at all; just confirm the file was written and where. If something looks genuinely wrong, describe it in plain language without the flag identifier or technical specifics — e.g. "Uh oh, the speaker notes may not be exporting properly." rather than "I received the no_speaker_notes flag", or "A couple of slides may have captured identically — let me fix navigation and retry." rather than quoting `duplicate_adjacent`.
