...
All files in this directory are vendored verbatim (no local modifications) so the skill works offline. Sync them when bumping versions, and keep the pinned CDN fallback tags in `system-prompt.md` ("React + Babel") and `scripts/lib/ds-prompt.mjs` in step.
...
| `fig-materialize.mjs` | Offline Figma `.fig` decoder + materializer, vendored from the upstream `baoyu-design` repo (`skills/baoyu-design/scripts/vendor/fig-materialize.mjs`); embeds an inlined zstd WASM decoder | https://github.com/JimLiu/baoyu-design | MIT (per upstream repo license) |
