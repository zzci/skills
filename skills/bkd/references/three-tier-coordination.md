...
3. Run health and capacity checks; record `canStartNewExecution` (and
   ...
   ```
4. Only if the user named an engine or model: resolve it to an exact id via
   `/engines/available` (see [Model Selection](#model-selection-user-directed-only)).
   Otherwise skip this step — every issue is created without `engineType`/
   `model` and the defaults apply.
   ```bash
...
## Model Selection (user-directed only)

**Default: do not choose a model.** Unless the user explicitly names an engine
or model — for the whole campaign, for a specific L2, or for a class of L3s —
create every L2 and L3 **without** `engineType`/`model`, so BKD applies the
project default and then the server's engine default. L1 never picks a model
on its own judgment, never "upgrades" a task it considers hard, and never
switches models on a retry; L2 inherits the same rule for its L3s. If a model
choice seems to matter, say so to the user and ask — do not decide.

When the user does specify a model, the mechanics:
...
- Omitted `model` resolves through project/engine defaults; that is the
  normal state of a campaign.

Resolving the user's wording to exact ids (as verified on BKD v0.1.0;
re-check per server with `/engines/available`):

| User says | `engineType` | `model` |
|-----------|--------------|---------|
| "Claude Fable 5.1" | `claude-code` | `claude-fable-5-1` |
| "Opus 5" | `claude-code` | `claude-opus-5` |
| "Codex 5.6 Sol" | `codex` | `gpt-5.6-sol` |

Use a `[1m]` variant (e.g. `claude-fable-5-1[1m]`) only when the user asks for
the 1M-context model by name.

How a user-specified directive flows through the pattern:

- **L1** records the directive verbatim in the dispatch package (which tier,
  which L2s, which class of L3s it covers) and pins an L2's engine/model at
  create time only when the directive covers that L2.
- **L2** applies the directive literally at L3 creation and records the
  assigned engine/model per subtask in the plan snapshot. Anything the
  directive does not cover stays at the default; L2 does no profile-based
  classification of its own.
- **Retries keep the model.** A red L3 is retried on the same engine/model
  unless the user's directive explicitly names an escalation model.
...
- **User confirmation gate (HARD RULE — no exceptions).** L1 may NOT create any
  ...
  1. Draft the dispatch package: classification (new vs continuation), shared
     ...
     out-of-scope, cross-L2 dependencies, engine/model only if the user
     specified one }`, plus open questions.
...
# No engineType/model: the project/server default applies. Only when the user
# named a model for this L2, add `--arg engine ... --arg model ...` and
# `engineType:$engine,model:$model` (ids from /engines/available).
L2=$(jq -n \
  --arg title "$L2_TITLE" \
...
## Model Policy
(L1: include this section only when the user explicitly specified a model for
L3s; delete it otherwise. Without it, create every L3 with no engineType/model
so the project/server default applies, and never pick one yourself.)
User directive: {verbatim user wording}
Applies to: {all L3s | the L3s matching: ...}
engineType={engineType} model={modelId}   (ids already verified by L1)
Retries keep the same model unless stated here: {none | escalation model}
Use ONLY these ids — an unknown id silently falls back to the engine default.
Record each L3's assigned engine/model in the plan snapshot.
...
4. Evaluate completions immediately (do not batch) via `logs/filter` — classify green/yellow/red using only the logs-filter assessment in `quality-review.md`; this pattern relies on the L3's project checks rather than any specific external review skill:
   ...
   - **red** → if the L3 is still running, stop → verify review → follow-up
     (see [Loop Engine](#loop-engine)). Retry ≤ `N=2` on the same
     engine/model (attach a `model` to the rework follow-up only if the user's
     Model Policy names an escalation model); on exceed, set DAG state
...
# No engineType/model: the project/server default applies. Only when the
# dispatch package's Model Policy (a user directive) covers this L3, add
# `--arg engine ... --arg model ...` and `engineType:$engine,model:$model`.
SUB=$(jq -n \
  --arg title "$SUB_TITLE" \
  --arg campaign "campaign:{campaignId}" \
...
16. **Models default unless the user says otherwise** — create L2/L3 issues
    without `engineType`/`model` so project/server defaults apply; neither L1
    nor L2 chooses a model on its own, and retries keep the same model. Only
    an explicit user directive pins an engine/model, using ids verified via
    `/engines/available` (unknown ids silently fall back; a model change is
    rejected with 409 while the session is running — stop first). See [Model
    Selection](#model-selection-user-directed-only).
