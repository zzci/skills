...
Create each versioned migration stub with `sqlx migrate add <name>`, then author its SQL body. SQLx allocates and tracks the migration; it does not derive SQL from Rust models. Never edit a migration already applied to a shared environment.
...
When ActiveRecord-style entities + relations dominate the design. Create the migration crate/scaffolding with `sea-orm-cli`, then implement the generated migration modules. Pair with rustls feature flags; reject `native-tls`.
